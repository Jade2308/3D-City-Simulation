...
class Road:
    ...
    def draw_road_segment(self, x, z, width, length, orientation):
        ...
        if orientation == 'horizontal':
            glVertex3f(x, 0.25, z - width/2)
            glVertex3f(x + length, 0.25, z - width/2)
            glVertex3f(x + length, 0.25, z + width/2)
            glVertex3f(x, 0.25, z + width/2)
        else:  # vertical
            glVertex3f(x - width/2, 0.25, z)
            glVertex3f(x + width/2, 0.25, z)
            glVertex3f(x + width/2, 0.25, z + length)
            glVertex3f(x - width/2, 0.25, z + length)
        ...
        if orientation == 'horizontal':
            ...
            for i in range(num_dashes):
                ...
                end_x = start_x + 1
                if end_x <= x + length:
                    glVertex3f(start_x, 0.26, z)
                    glVertex3f(end_x, 0.26, z)
        else:  # vertical
            # Dashed center line
            num_dashes = int(length / 2)
            for i in range(num_dashes):
                ...
                end_z = start_z + 1
                if end_z <= z + length:
                    glVertex3f(x, 0.26, start_z)
                    glVertex3f(x, 0.26, end_z)
